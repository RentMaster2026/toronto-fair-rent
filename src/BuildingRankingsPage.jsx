...
//
// Props:
//   city        TORONTO_CITY object from hoodData.js
//   buildings   TORONTO_BUILDINGS array from buildingData.js
//   hood        optional - TORONTO_HOODS[key] object for neighbourhood filtering
...
function ScoreMethodology() {
  ...
  const cats = [
    ...
    { pts: "15 pts", label: "Value for location", desc: "Looks at whether the rent makes sense for where the building sits. Factors include TTC subway and streetcar access, walkability, distance to downtown, and local demand." },
    { pts: "15 pts", label: "Building features", desc: "Scores the amenities Toronto renters actually care about: in-unit laundry, parking, air conditioning, gym, concierge, elevator, balcony, pet friendliness, and storage." },
    ...
    { pts: "10 pts", label: "Affordability pressure", desc: "Looks at whether the building is accessible relative to local income and typical renter budgets in Toronto." },
  ];
  return (
...
function HoodComparisonTable({ hoods, cityBases, onHoodClick }) {
  const rows = Object.entries(hoods).map(([key, h]) => {
    const base1br = cityBases["1br"] ?? 2400;
...
const CITY_FAQS = [
  {
    q: "How does FairRent Canada score Toronto rental buildings?",
  ...
  {
    q: "Can a high-priced building in Yorkville or King West still score well?",
    a: "Yes. We do not punish a building just because it is expensive. A luxury building in Yorkville can still score well if its rent is fair compared to similar luxury buildings in the same neighbourhood. The score compares apples to apples, not luxury condos to walk-up apartments.",
  },
  {
    q: "How is this different from Yelp or Google Reviews?",
    a: "FairRent Canada focuses on rent data, not reviews. We collect anonymous rent amounts and building details from current and past tenants to build a data picture of what Toronto renters actually pay. We do not publish reviews, complaints, or accusations about landlords.",
  },
  {
...
export default function BuildingRankingsPage({
  ...
  const pageTitle = isHoodPage
    ...
    : `Toronto Rental Building Rankings`;
  ...
  const pageSubtitle = isHoodPage
    ...
    : `Compare large rental buildings across Toronto using anonymous renter submissions, public rental listings, and local market data. Every score is an estimate, not an official rating.`;
      ...
      <div style={{ minHeight:"100vh", background:"var(--bg)" }}>
        ...
        <div className="br-wrap">
          ...
          <div className="br-crumb">
            <button onClick={onBack}>Toronto</button>
            {isHoodPage && (
              <>
          ...
          <section className="br-hero">
            <div className="br-eyebrow">FairRent Canada - Toronto</div>
            ...
            <p style={{ fontSize:12, color:"var(--t3)", lineHeight:1.6 }}>
              {scoresReady
                ? `Showing ${totalBuildings} known rental building${totalBuildings !== 1 ? "s" : ""} in ${isHoodPage ? hood.name : "Toronto"}. Scores improve as renters submit data.`
            ...
            <div className="br-body">
              <p>
                {isHoodPage
                  ...
                  : "These are the purpose-built and condo-rental buildings FairRent Canada is tracking in Toronto. Each score is based on anonymous renter submissions and public market data. Buildings with fewer than 5 submissions show as Limited data."}
              </p>
            </div>
            ...
            {!scoresReady ? (
              ...
              <div className="buildings-grid">
                {pageBuildings.map(b => {
                  ...
                  // Hood name lookup: b.neighbourhood is the hood key (e.g. "annex")
                  ...
                  if (subs.length >= MIN_SUBS_FOR_AVG) {
                    ...
                    const bench = Math.round((city.bases?.["1br"] ?? 2400) * hoodMult);
                    if (subs.length >= MIN_SUBS_FOR_SCORE) {
                      const fullScore = calcBuildingScore({
                        building: b,
                        submissions: subs,
                        cityBaseBedroom: city.bases?.["1br"] ?? 2400,
                        hoodMult,
                      });
          ...
          <section className="br-section">
            <div className="help-card">
              ...
              <p className="help-microcopy">
                Anonymous - No signup - Your submission helps other Toronto renters understand what is fair.
              </p>
            </div>
          </section>
          ...
          {!isHoodPage && allHoods && (
            <section className="br-section">
              ...
              <h2 className="br-h2">Toronto rent by neighbourhood</h2>
              <div className="br-body">
                <p>
                  Rent varies dramatically across Toronto. Yorkville, Forest Hill, and the Harbourfront
                  typically run well above the city average. Scarborough, Etobicoke, and East York offer
          ...
          <section className="br-section">
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))", gap:10, marginTop:10 }}>
              {[
                ...
                { label: "Browse Toronto neighbourhoods", href: "https://fairrent.ca/map?city=toronto" },
        ...
        <Footer compact={true} citySuffix="Toronto" />
      </div>
    </>
  );
}
