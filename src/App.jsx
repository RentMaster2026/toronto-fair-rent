...
const ACCENT_FOCUS = "rgba(37,99,235,0.15)";
...
const HOODS = {"Annex":1.1,"Bloorcourt":1,"Chinatown":0.95,"Davisville":1.08,"Distillery District":1.2,"Downtown Core":1.18,"East End":0.94,"East York":0.92,"Etobicoke":0.87,"Forest Hill":1.32,"Greektown":1.02,"Junction":0.97,"Kensington Market":0.96,"Lawrence Park":1.25,"Leaside":1.14,"Leslieville":1.04,"Liberty Village":1.12,"Little Italy":1.05,"Midtown":1.07,"North York":0.9,"Parkdale":0.93,"Queen West":1.15,"Riverside":1.01,"Rosedale":1.35,"Scarborough":0.82,"St. Lawrence":1.09,"Swansea":1.03,"Weston":0.86,"Willowdale":0.89,"Yorkville":1.3};
...
const CSS = `
  ...
  *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
  ...
  .toggle-btn.on { border-color:#bfdbfe; background:#eff6ff; }
  ...
  .opt-btn.on { border-color:#bfdbfe; background:#eff6ff; color:#2563eb; font-weight:600; }
...
export default function App() {
  ...
  function shareText(){
    ...
    return `Toronto Rent Check: ${result.todayPct>0?"+":""}${result.todayPct}% vs market for a ${u} in ${hood}. ${SHARE_URL}`;
  }
    ...
    <div style={{minHeight:"100vh"}}>
      ...
      <header style={{background:"var(--nav)",borderBottom:"1px solid rgba(255,255,255,.06)",position:"sticky",top:0,zIndex:100}}>
        <div style={{maxWidth:"var(--max)",margin:"0 auto",padding:"0 20px",display:"flex",alignItems:"center",justifyContent:"space-between",height:56}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            <span style={{fontFamily:"var(--sans)",fontSize:14,fontWeight:600,color:"#f8fafc",letterSpacing:"-.01em"}}>Toronto Rent Calculator</span>
          </div>
          {countLoaded&&(
      ...
      <main style={{maxWidth:"var(--max)",margin:"0 auto",padding:"40px 20px 80px"}}>
          ...
          <>
            <div style={{marginBottom:32}}>
              <h1 style={{fontFamily:"var(--sans)",fontSize:"clamp(24px,4vw,36px)",fontWeight:700,lineHeight:1.15,letterSpacing:"-.02em",marginBottom:12,color:"var(--t1)"}}>
                Is your Toronto rent<br/>
                <span style={{fontStyle:"italic",color:ACCENT}}>actually fair?</span>
              </h1>
              <p style={{fontSize:15,color:"var(--t2)",lineHeight:1.7,maxWidth:440}}>
                Check if your Toronto rent is fair. Based on CMHC data and real renter submissions. Free. Anonymous. No account needed.
              </p>
            </div>
            ...
            <div className="card" style={{padding:"28px 24px",display:"flex",flexDirection:"column",gap:20}}>
              ...
              {hood&&unitType&&benchReady&&previewBench!=null&&(
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10,padding:"12px 16px",background:ACCENT_LIGHT,border:`1px solid ${ACCENT_BORDER}`,borderRadius:"var(--r-md)"}}>
                  <div>
                    <div style={{fontFamily:"var(--mono)",fontSize:10,color:"var(--t3)",textTransform:"uppercase",letterSpacing:".07em",marginBottom:3}}>Toronto benchmark — {hood}</div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            <div className={`card fade-up d1`} style={{padding:"26px 24px",background:result.verdict.bg,borderColor:result.verdict.border}}>
              <div style={{fontFamily:"var(--mono)",fontSize:10,color:result.verdict.color,letterSpacing:".1em",textTransform:"uppercase",marginBottom:10,opacity:.8}}>
                Toronto · {hood} · {UNITS.find(u=>u.key===unitType)?.label}
              </div>
            ...
            {shareOpen&&(
              <div className="card fade-up" style={{padding:"18px 20px"}}>
                ...
                <div className="gshare" style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8}}>
                  ...
                  {navigator.share
                    ? <button className="share-btn" onClick={()=>navigator.share({title:"Toronto Rent Calculator",text:shareText(),url:SHARE_URL}).catch(()=>{})} style={{background:"var(--bg)",border:"1px solid var(--border)",color:"var(--t2)"}}>More</button>
        ...
        <div style={{marginTop:40,padding:"24px",background:"var(--bg-card)",border:"1px solid var(--border)",borderRadius:"var(--r-lg)",boxShadow:"var(--sh)"}}>
          <div style={{fontFamily:"var(--mono)",fontSize:10,color:"var(--t3)",textTransform:"uppercase",letterSpacing:".1em",marginBottom:16}}>Toronto rent at a glance (2025)</div>
          <div>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",borderBottom:"1px solid var(--border)"}}><span style={{fontSize:13,color:"var(--t2)"}}>1-bedroom average</span><span style={{fontFamily:"var(--mono)",fontSize:13,fontWeight:500,color:"var(--t1)"}}>$2,183/mo</span></div>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",borderBottom:"1px solid var(--border)"}}><span style={{fontSize:13,color:"var(--t2)"}}>2-bedroom average</span><span style={{fontFamily:"var(--mono)",fontSize:13,fontWeight:500,color:"var(--t1)"}}>$2,690/mo</span></div>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",borderBottom:"1px solid var(--border)"}}><span style={{fontSize:13,color:"var(--t2)"}}>Vacancy rate (2025)</span><span style={{fontFamily:"var(--mono)",fontSize:13,fontWeight:500,color:"var(--t1)"}}>1.5%</span></div>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",borderBottom:"1px solid var(--border)"}}><span style={{fontSize:13,color:"var(--t2)"}}>Highest neighbourhood</span><span style={{fontFamily:"var(--mono)",fontSize:13,fontWeight:500,color:"var(--t1)"}}>Rosedale, Forest Hill (+30%)</span></div>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",borderBottom:"1px solid var(--border)"}}><span style={{fontSize:13,color:"var(--t2)"}}>Most affordable</span><span style={{fontFamily:"var(--mono)",fontSize:13,fontWeight:500,color:"var(--t1)"}}>Scarborough, Weston (-15%)</span></div>
