...
import { TORONTO_HOODS, TORONTO_CITY } from "./hoodData";
import { TORONTO_BUILDINGS, HOOD_NAME_TO_KEY, resolveBuildingName, getBuildingById } from "./buildingData";
...
const COOLDOWN_KEY    = "toronto_fair_rent_last_submit";
const COOLDOWN_MS     = 60_000;
const ACCENT          = "#0e7c3a";
const ACCENT_BG       = "#f0f7f2";
const SHARE_URL       = "https://torontofairrent.ca";
...
const STRINGS = {
  en: {
    ...
    heroSubStrong:`${CITY_NAME} renters need better transparency.`,
    heroSubRest:`Submit your rent anonymously in under a minute to help build the city's tenant-powered rent database.`,
  ...
  fr: {
    ...
    // Community hero
    heroEyebrow:"Communauté de transparence des loyers à Toronto",
    heroTitle:"Voyez ce que paient vraiment les locataires de Toronto.",
    heroSubStrong:"Toronto a besoin de plus de transparence sur les loyers.",
    heroSubRest:"Partagez votre loyer anonymement en moins d'une minute pour bâtir la base de données des loyers de la ville.",
    heroCtaPrimary:"Partager mon loyer à Toronto",
    heroCtaSecondary:"Explorer les quartiers",
    heroTickerPre:"Rejoint par",
    heroTickerPost:"locataires anonymes à Toronto et en croissance.",
...
const CSS = `
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
  ...
  .yn-btn{padding:10px 12px;border:1px solid var(--border);background:var(--white);color:var(--t2);font-size:13px;font-weight:600;cursor:pointer;text-align:center;border-radius:6px;}
  .yn-btn:hover{border-color:var(--t3);}
  .yn-btn.on{border-color:var(--accent);background:var(--accent-soft);color:var(--accent);}
  ...
  .result-header{padding:14px 16px;border-bottom:1px solid var(--border-soft);background:#fbfcfd;display:flex;justify-content:space-between;align-items:flex-start;gap:10px;flex-wrap:wrap;}
  ...
  .bldg-est-mini{font-size:11.5px;color:var(--t3);line-height:1.5;margin:0;}
  ...
  .btn-secondary{padding:10px 14px;background:transparent;border:1px solid var(--border);color:var(--t2);font-size:13px;font-weight:600;cursor:pointer;text-align:center;border-radius:6px;}
...
// always shows low confidence, and uses the safeCopy text written for
// careful, non-accusatory wording.
  ...
  return (
    <div className="bldg-est-card" role="region" aria-label="Building estimate">
      ...
      <div className="bldg-est-conf">
        <span className="bldg-est-dot" /> Low confidence · Based on available market signals
      </div>
      <p className="bldg-est-copy">
        {est.safeCopy || `This is an early estimate based on available market signals. It will become more accurate as renters submit anonymous rent data for this building.`}
      </p>
      <p className="bldg-est-mini">
        Needs more anonymous renter submissions before it becomes a verified building score.
      </p>
    ...
    <div className="result-panel">
      ...
      <div className="result-thanks">
        ...
        <div className="result-thanks-text">
          Your anonymous rent helps the next renter in Toronto understand the market.
...
export default function App() {
  ...
  const [preNov2018, setPreNov2018] = useState(savedDraft.preNov2018 ?? null);
  ...
  useEffect(() => {
    try {
      sessionStorage.setItem(FORM_KEY, JSON.stringify({
        hood, unitType, rent, moveInYear, sqft, parking, utilities, preNov2018,
        building,
      }));
    } catch {}
  }, [FORM_KEY, hood, unitType, rent, moveInYear, sqft, parking, utilities, preNov2018, building]);
  ...
  useEffect(() => {
    function parsePath(pathname) {
      ...
      if (parts.length === 2 && parts[1] === 'building-rankings') {
        const hoodEntry = Object.entries(TORONTO_HOODS).find(([, h]) => h.slug === parts[0]);
        if (hoodEntry) { setShowBuildingRankings(hoodEntry[0]); setShowHood(null); return; }
      }
      // /{hood-slug}
      const found = Object.entries(TORONTO_HOODS).find(([, h]) => h.slug === path);
  ...
  function navToBuildingRankings(hoodKey) {
    if (!hoodKey) {
      ...
      const slug = TORONTO_HOODS[hoodKey]?.slug;
  ...
  if (showBuildingRankings) {
    ...
    return (
      <BuildingRankingsPage
        city={TORONTO_CITY}
        buildings={TORONTO_BUILDINGS}
        hood={hoodKey ? TORONTO_HOODS[hoodKey] : null}
        hoodKey={hoodKey}
        allHoods={TORONTO_HOODS}
        onBack={() => {
  ...
  return (
    <>
      ...
      <div style={{ minHeight:"100vh", background:"var(--bg)" }}>
        <Nav
          ...
          actions={{
            ...
            onNeighbourhoods: () => { window.location.href = "https://fairrent.ca/map?city=toronto"; },
        ...
        <div className="page-wrap">
          ...
          {/* Community hero — Toronto-focused Glassdoor framing.
          ...
          <section className="city-hero">
            ...
            <div className="city-hero-actions">
              ...
              <a className="city-hero-btn city-hero-btn-ghost" href="https://fairrent.ca/map?city=toronto" onClick={()=>window.frc?.track?.('city_hero_explore_click',{city:CITY})}>{t('heroCtaSecondary')}</a>
          ...
          <div className="hood-section">
            ...
            <div className="hood-pills">
              {Object.keys(TORONTO_HOODS).map(key => {
                const h = TORONTO_HOODS[key];
                return (
                  <a
                    key={key}
                    href={"/" + h.slug}
            ...
            <div style={{ marginTop:14 }}>
              ...
              >
                View Toronto building rankings
                ...
                <div className="form-body">
                  ...
                  <div className="f-row">
                    <div>
                      ...
                      <div className="field-note">{t('rentMicro')}</div>
                    </div>
                    <div>
                  ...
                  <div>
                    ...
                    <BuildingAutocomplete
                      ...
                      placeholder={hood
                        ? `Search ${hood} buildings or any Toronto address`
      ...
      <Footer compact={false} citySuffix={"Toronto"} />
    </>
  );
}
