...
const NEIGHBORHOODS = [
  "Downtown Core","King West","Queen West","Distillery District","Liberty Village",
  "Waterfront","Yorkville","Annex","Rosedale","Forest Hill",
  "Lawrence Park","Leaside","Davisville","Midtown","North York",
  "Scarborough","Etobicoke","East York","Leslieville","Riverside",
  "Parkdale","Junction","Bloorcourt","Kensington Market","Chinatown",
  "Little Italy","Greektown","Danforth","East End","Willowdale",
];
...
const BASE_AVERAGES = { bachelor: 1800, "1br": 2400, "2br": 3100, "3br": 3900, "3plus": 4800 };

const HOOD_MULTIPLIERS = {
  "Rosedale": 1.35, "Forest Hill": 1.32, "Yorkville": 1.30,
  "Lawrence Park": 1.25, "King West": 1.22, "Distillery District": 1.20,
  "Downtown Core": 1.18, "Waterfront": 1.18, "Queen West": 1.15,
  "Leaside": 1.14, "Liberty Village": 1.12, "Annex": 1.10,
  "Davisville": 1.08, "Midtown": 1.07, "Little Italy": 1.05,
  "Leslieville": 1.04, "Greektown": 1.02, "Riverside": 1.01,
  "Bloorcourt": 1.00, "Danforth": 0.98, "Junction": 0.97,
  "Kensington Market": 0.96, "Chinatown": 0.95, "East End": 0.94,
  "Parkdale": 0.93, "East York": 0.92, "North York": 0.90,
  "Willowdale": 0.89, "Etobicoke": 0.87, "Scarborough": 0.82,
};
...
function getMarket(neighborhood, unitType, moveInYear, parking, utilities) {
  const base     = BASE_AVERAGES[unitType] || 2400;
...
function getInsight(pct, moveInYear) {
  ...
  if (pct > 5)    return { head: "Slightly above market.", body: "Premium amenities, included utilities, parking, or a newer building can justify above-market rents in Toronto. Compare what's included carefully before drawing conclusions.", link: null };
  if (pct >= -5)  return { head: "You're right at market rate.", body: "Your rent aligns well with Toronto averages for this unit type and neighbourhood — a useful baseline for any upcoming lease renewal conversation.", link: null };
  ...
  return { head: "You have a notably strong deal.", body: "You're well below today's Toronto market. Protect this — understand your renewal rights, and be cautious about voluntary moves that would reset your rent.", link: { t: "Before You Move: Know Your Rights", u: "https://www.ontario.ca/page/renting-ontario-your-rights" } };
}
...
export default function App() {
  ...
  function getShareText() {
    ...
    return `Toronto Fair Rent: I'm paying ${result.todayPct > 0 ? "+" : ""}${result.todayPct}% vs today's market for a ${unit} in ${form.neighborhood}. torontofairrent.ca`;
  }

  function copyLink() {
    navigator.clipboard?.writeText('https://torontofairrent.ca');
    setToast(true);
  ...
  function nativeShare() {
    if (navigator.share) {
      navigator.share({ title: 'Toronto Fair Rent', text: getShareText(), url: 'https://torontofairrent.ca' }).catch(() => {});
    }
  }
  ...
  return (
    <div style={{ minHeight: "100vh", width: "100%", background: "var(--bg)", fontFamily: "'Source Serif 4', serif", overflowX: "hidden" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        select:focus, input:focus {
          ...
          box-shadow: 0 0 0 3px rgba(29,52,97,.13) !important;
        }
      ...
      {/* ── Masthead ── */}
      <header style={{ background: "var(--ink)", color: "var(--paper)", borderBottom: "4px solid #c8a951", width: "100%" }}>
        ...
        <div style={{ padding: "20px 28px 18px", display: "flex", justifyContent: "space-between", alignItems: "flex-end", gap: 16 }}>
          <div>
            ...
            <div style={{ fontFamily: "'Source Serif 4', serif", fontSize: 14, color: "rgba(255,255,255,.5)", marginTop: 6, fontStyle: "italic" }}>
              A community rent transparency tool for the Greater Toronto Area
            </div>
          </div>
          <div style={{ textAlign: "right", flexShrink: 0 }}>
            <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 30, fontWeight: 700, color: "#c8a951", lineHeight: 1 }}>
      ...
      <main style={{ maxWidth: 700, margin: "0 auto", padding: "32px 20px 72px" }}>
          ...
          <div style={{ background: "var(--paper)", borderRadius: 10, boxShadow: "0 2px 28px rgba(0,0,0,.08)", overflow: "hidden" }}>
            <div style={{ padding: "22px 26px 18px", borderBottom: "1px solid var(--rule)" }}>
              ...
              <p style={{ fontFamily: "'Source Serif 4', serif", fontSize: 14, color: "var(--ink-muted)", marginTop: 5, lineHeight: 1.65 }}>
                Enter your details for an instant comparison against Toronto market rates.
            ...
            <div style={{ padding: "22px 26px 26px", display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <div className="g2" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                <Field label="Monthly Rent (CAD)" error={errors.rent}>
                  <input type="number" placeholder="e.g. 2400" value={form.rent} onChange={set("rent")} style={inpStyle(errors.rent)} />
                </Field>
              ...
              {shareOpen && (
                <div style={{ background: "var(--paper-tint)", border: "1px solid var(--rule)", borderRadius: 10, padding: "16px 18px", display: "flex", flexDirection: "column", gap: 10 }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                    <a href={`https://www.reddit.com/submit?url=https://torontofairrent.ca&title=${encodeURIComponent(getShareText())}`} target="_blank" rel="noopener noreferrer" style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: "10px", background: "#ff4500", color: "white", borderRadius: 7, fontFamily: "'DM Mono', monospace", fontSize: 11, fontWeight: 600, textDecoration: "none", letterSpacing: ".04em" }}>Reddit</a>
        ...
        <p style={{ textAlign: "center", marginTop: 24, fontFamily: "'DM Mono', monospace", fontSize: 10, color: "var(--ink-muted)", letterSpacing: ".06em", lineHeight: 1.9 }}>
          Benchmarks use Toronto-wide estimates with neighbourhood multipliers and an annual inflation model.
